...
class WorkoutServiceTest {
    ...
    void generateShouldBuildLoseWeightPlan() {
        when(profileRepository.findByUser_EmailIgnoreCase("user@mail.com"))
                .thenReturn(Optional.of(profile("lose_weight", 2)));
    ...
    void generateShouldBuildMaintainPlan() {
        when(profileRepository.findByUser_EmailIgnoreCase("user@mail.com"))
                .thenReturn(Optional.of(profile("maintain", 4)));
    ...
    void generateShouldBuildGainMusclePlan() {
        when(profileRepository.findByUser_EmailIgnoreCase("user@mail.com"))
                .thenReturn(Optional.of(profile("gain_muscle", 5)));
    ...
    @Test
    void generateShouldReturnMinimumThreeDays() {
        when(profileRepository.findByUser_EmailIgnoreCase("user@mail.com"))
                .thenReturn(Optional.of(profile("gain_muscle", 1)));
    ...
    void generateShouldFailWithoutProfile() {
        ...
        assertThrows(
                ...
                () -> workoutService.generate("user@mail.com")
        );
    }

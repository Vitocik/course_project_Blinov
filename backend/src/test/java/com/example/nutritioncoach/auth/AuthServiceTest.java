...
class AuthServiceTest {
    ...
    void registerShouldCreateUserAndReturnAuthResponse() {
        ...
        when(userAccountRepository.existsByEmailIgnoreCase("test@email.com"))
                .thenReturn(false);
        ...
        when(userProfileRepository.findByUser_EmailIgnoreCase("test@email.com"))
                .thenAnswer(invocation -> {
                    UserAccount user =
                            new UserAccount(
                                    "test@email.com",
        ...
        when(progressRepository.findByUser_EmailIgnoreCaseOrderByEntryDateDesc("test@email.com"))
                .thenReturn(List.of(
                        progress(
                                "test@email.com",
                                72.4,
        ...
        when(jwtService.generateToken("test@email.com", "USER"))
    ...
    @Test
    void registerShouldUseEmailPrefixWhenNameEmpty() {
        ...
        when(userAccountRepository.existsByEmailIgnoreCase("user@mail.com"))
                .thenReturn(false);
        ...
        when(userProfileRepository.findByUser_EmailIgnoreCase("user@mail.com"))
                .thenReturn(Optional.empty());

        when(progressRepository.findByUser_EmailIgnoreCaseOrderByEntryDateDesc("user@mail.com"))
                .thenReturn(List.of());

        when(jwtService.generateToken("user@mail.com", "USER"))
                .thenReturn("token");
    ...
    void loginShouldReturnTokenWhenPasswordMatches() {
        ...
        when(userAccountRepository.findByEmailIgnoreCase("user@mail.com"))
    ...
    void loginShouldRejectUnknownUser() {
        ...
        when(userAccountRepository.findByEmailIgnoreCase("unknown@mail.com"))
                .thenReturn(Optional.empty());
    ...
    private static ProgressEntry progress(
            String email,
            double weight,
            LocalDate date,
            String note
